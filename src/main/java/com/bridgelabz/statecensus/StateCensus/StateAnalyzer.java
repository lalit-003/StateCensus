...
public class StateAnalyzer {
	...
	public int loadCensusData(String filePath) throws ExceptionStateCensus {
		try {
			...
			List<CSVStateCensus> list = csvBuilder.getList(reader, CSVStateCensus.class);
	...
	public int loadCodeData(String filePath) throws ExceptionStateCensus {
		try {
			Reader reader = Files.newBufferedReader(Paths.get(filePath));
			ICSVBuilder csvBuilder = BuilderFactoryCSV.generateBuilder();
			List<CSVStateCode> list = csvBuilder.getList(reader, CSVStateCode.class);
			int listSize = list.size();
			BufferedReader bufferReader = new BufferedReader(new FileReader(filePath));
			String line = "";
			int count = 0;
			while ((line = bufferReader.readLine()) != null) {
				...
				if (count != 0 && (headers[0].equals("") || headers[1].equals("") || headers[2].equals(""))
						&& !headers[3].equals("'")) {
					throw new ExceptionStateCensus("Invalid delimiter  in StateCode file",
							ExceptionType.INVALID_DELIMITER);
				}
				if (count == 0 && (!headers[0].equals("SrNo") || !headers[1].equals("State Name")
